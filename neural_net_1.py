...
class TwoLayerNet(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, std=1e-4):
        ...
        가중치와 절편 초기화: Weights are initialized to small random values and
        ...
        """
        self.params = {} # parameter 저장하는 dictionary
        self.params['W1'] = std * np.random.randn(input_size, hidden_size)
        # [inputsize x hiddensize]크기의 행렬. 가중치가 평균이 0이고 표준편차가 std인 정규분포를 따르도록 초기화 
        self.params['b1'] = np.zeros(hidden_size) # bias는 0으로 초기화
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # https://data-make.tistory.com/117 축 지정, 차원 유지
        scores_exp = np.exp(scores) # score값들을 모두 지수화
        softmax_matrix = scores_exp / np.sum(scores_exp, axis=1, keepdims=True) # normalize 해준다
        loss = np.sum(-np.log(softmax_matrix[np.arange(N), y])) # -log를 취해서 loss값을 구한다
        loss /= N
        loss += reg * (np.sum(W2 * W2) + np.sum( W1 * W1 )) # regularization - L2 규제
